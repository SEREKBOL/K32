...
Telegram: @TarganJack_channel Or @@TarganJack_bot Chat
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            # Verify Access Key - 0615 and 0318 are admin keys
            entered_normalized = access_key.replace("\n", "").replace(" ", "").strip()
            
            # Admin keys: "0615" and "0318"
            if entered_normalized == "0615" or entered_normalized == "0318":
                key_found = True
                user_id_ref = "ADMIN"
                is_unlimited = True
            else:
        ...
        while True:
            ...
            balance = 999999999 if is_unlimited else db.get(user_id_ref, {}).get('balance', 0)
            ...
            if choice in PRICES:
                ...
                if choice == "1":
                    ...
                    if res_act.get('ok'):
                        print_rainbow("{%} KING RANK granted successfully")
                    else:
                        print_rainbow("[×] Failed to grant KING RANK")

                elif choice == "2":
                    print_rainbow("{%} Change your email")
                    new_e = input(rainbow_text("Your new email: "))
                    res_act = client.make_request("change_email", {"account_auth": auth_token, "new_email": new_e})
                    if res_act.get('ok'):
                        print_rainbow("{%} Email changed successfully")
                    else:
                        print_rainbow("[×] Failed to change email")

                elif choice == "3":
                    print_rainbow("{%} Change your password")
                    ...
                    if res_act.get('ok'):
                        ...
                        print_rainbow("[×] Failed to change password")

                elif choice == "4":
                    re = input(rainbow_text("Register Email: "))
                    rp = input(rainbow_text("Register Password: "))
                    print_rainbow("{%} Registering new account")
                    ...
                    if res_act.get('ok'):
                        ...
                        print_rainbow("[×] Failed to register")
                ...
                if ans == 'y':
                    print_rainbow("Account signed out successfully")
                    time.sleep(1)
                    break
